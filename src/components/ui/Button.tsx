...
export const Button: React.FC<ButtonProps> = ({
  ...
  const base = "group inline-flex items-center justify-center font-bold rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#F97316]/40 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer active:scale-[0.98]";
  ...
  return (
    ...
    >
      {isLoading ? (
        ...
        <>
          {icon && iconPosition === 'left' && <span className="shrink-0 transition-transform duration-200 ease-out group-hover:-translate-x-0.5">{icon}</span>}
          <span>{children}</span>
          {icon && iconPosition === 'right' && <span className="shrink-0 transition-transform duration-200 ease-out group-hover:translate-x-1">{icon}</span>}
        </>
      )}
    </button>
  );
